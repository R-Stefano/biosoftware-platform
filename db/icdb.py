...
class IonChannelDatabase(object):
    ...
    def create_tissue_table(self):
        ...
        curs.execute("""
            CREATE TABLE Tissue(
                ...
                IsCancerTissue TEXT NOT NULL)
            """)
        conn.commit()

    def add_tissue(
            self, name, bto_id='', parent_tissue_name='',
            is_cancer_tissue='N'):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("""
            INSERT INTO Tissue(Name, BTOId, ParentTissueName, IsCancerTissue)
            VALUES (?, ?, ?, ?)
            """, (name, bto_id, parent_tissue_name, is_cancer_tissue))
        conn.commit()

    def get_tissues(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("SELECT * FROM Tissue")
        rows = curs.fetchall()
        return rows
    ...
    def get_normal_tissue_names(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("""
            SELECT Name
            FROM Tissue
            WHERE IsCancerTissue = 'N'
            """)
        row_list = curs.fetchall()
        return [elem[0] for elem in row_list]

    def get_cancer_tissue_names(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("""
            SELECT Name
            FROM Tissue
            WHERE IsCancerTissue = 'Y'
            """)
        row_list = curs.fetchall()
        return [elem[0] for elem in row_list]
    ...
    def get_ion_channels_uniprot(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("""
            SELECT
            ...
            FROM Protein p
            LEFT JOIN GoTerm gt
            ON (gt.UniProtAccNum=p.UniProtAccNum)
            WHERE p.IonChannelSubClass != '';
            """)
        row_list = curs.fetchall()
        return [elem for elem in row_list]
    ...
    def add_multiple_expression(self, records):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.executemany("""
            INSERT INTO Expression (
    ...
    def add_multiple_compound(self, records):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.executemany("""
            INSERT INTO Compound(
    ...
    def get_compounds(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("SELECT * FROM Compound")
        rows = curs.fetchall()
        return rows
    ...
    def update_multiple_interaction_zscores(self, records):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.executemany("""
            UPDATE Interaction
    ...
    def add_multiple_interaction(self, records):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.executemany("""
            INSERT INTO Interaction(
    ...
    def get_dbtissues(self):
        conn = self._get_conn()
        curs = conn.cursor()
        curs.execute("SELECT * FROM DBTissue")
        rows = curs.fetchall()
        return rows
    ...
    def print_db_stats(self):
        ...
        curs.execute("""
            ...
            """)
        row_list = curs.fetchall()
        gene_symbol_list = [elem[0] for elem in row_list]
        gene_symbol_list = list(set(gene_symbol_list))
        print ('Number of unique gene symbols = %d' % len(gene_symbol_list))
